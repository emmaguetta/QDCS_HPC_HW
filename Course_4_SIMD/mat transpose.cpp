...
inline void transAVX8x8_ps(__m256 tile[8])
{
    ...
    t[0] = _mm256_unpacklo_ps(tile[0], tile[1]);
    t[1] = _mm256_unpackhi_ps(tile[0], tile[1]);
    t[2] = _mm256_unpacklo_ps(tile[2], tile[3]);
    t[3] = _mm256_unpackhi_ps(tile[2], tile[3]);
    t[4] = _mm256_unpacklo_ps(tile[4], tile[5]);
    t[5] = _mm256_unpackhi_ps(tile[4], tile[5]);
    t[6] = _mm256_unpacklo_ps(tile[6], tile[7]);
    t[7] = _mm256_unpackhi_ps(tile[6], tile[7]);
    ...
    tile[0] = _mm256_permute2f128_ps(s[0], s[4], 0x20);
    tile[1] = _mm256_permute2f128_ps(s[1], s[5], 0x20);
    tile[2] = _mm256_permute2f128_ps(s[2], s[6], 0x20);
    tile[3] = _mm256_permute2f128_ps(s[3], s[7], 0x20);
    tile[4] = _mm256_permute2f128_ps(s[0], s[4], 0x31);
    tile[5] = _mm256_permute2f128_ps(s[1], s[5], 0x31);
    tile[6] = _mm256_permute2f128_ps(s[2], s[6], 0x31);
    tile[7] = _mm256_permute2f128_ps(s[3], s[7], 0x31);
}
...
int main(int argc, char **argv)
{
  ...
  // Transposer la matrice en utilisant tuiles de taille 8x8 avec AVX et transposition in-place
  {
    ...
    for (int repet = 0; repet < NREPET; repet++) {
      ...
      for (int i = 0; i < N; i += 8) {
        loadTile(tile, &A[i * N + i], N);
        transAVX8x8_ps(tile);
        storeTile(tile, &A[i * N + i], N);
      }
    }
    std::chrono::duration<double> time = std::chrono::high_resolution_clock::now() - start;
