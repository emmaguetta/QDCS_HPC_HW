/**
 * The parallel version is faster than the sequential version from N = 256.
 */
...
int main(int argc, char **argv)
{
  ...
  // Calculer b = A * x NREPET fois en sequentiel (b ecrase a chaque repetition)
  ...
  // Calculer b = A * x NREPET fois en parallele avec omp for (b ecrase a chaque repetition)
  ...
  double t_seq = tempsSeq.count() / NREPET;
  double t_par = tempsPar.count() / NREPET;
  double acceleration = t_seq / t_par;
  int p = omp_get_max_threads();
  double efficacite = acceleration / (double)p;
  std::cout << "Acceleration: " << acceleration << std::endl;
  std::cout << "Efficacite: " << efficacite << std::endl;

  // calculer b avec omp for si N >= 256
// Calculer b = A * x NREPET fois en parallele avec omp for (conditionnel)
start = std::chrono::high_resolution_clock::now();
...
std::chrono::duration<double> tempsParFor =
    std::chrono::high_resolution_clock::now() - start;

std::cout << std::scientific
          << "Temps d'execution parallele avec omp for (if dim>256): "
          << tempsParFor.count() / NREPET << "s\n";
