...
int main()
{
  ...
  for (int j = 0; j < N; j++) {          
    for (int i = 0; i < N; i++) {        
      if (i < j) L[i + j * N] = 0.0;     
      else       L[i + j * N] = (double)rand() / RAND_MAX;
    }
  }
  ...
  if (info != 0) {
    std::cerr << "Erreur Cholesky (dpotrf): info = " << info << std::endl;
    return -1;
  }
  ...
  char trans = 'N';
  dtrsv_(&uplo, &trans, &diag, &N, &A[0], &lda, &x[0], &incx);
  ...
  dgemv_(&transmv, &N, &N, &one, &Aorig[0], &lda, &x[0], &incx, &zero, &b2[0], &incx);

  double minus_one = -1.0;
  daxpy_(&N, &minus_one, &b[0], &incx, &b2[0], &incx);

  double err = dnrm2_(&N, &b2[0], &incx);
  std::cout << "||A x - b||_2 = " << err << std::endl;
